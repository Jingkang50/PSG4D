...
class LoadOutputsDKNet:
    """
    Process outputs of all images of a singel video from mask2former to inputs of unitracker.
    Modification from LoadImagesAndMaskObsMOTS: get_data
    """
    def __init__(self, video_name, cfg):
        prefix = cfg.segmentation_path
        if cfg.dataset == 'hoi4d':
            self.test_file_names = sorted(glob.glob(os.path.join(prefix, video_name, 'semantic_segmentation_label', '*' + 'npz')))
        elif cfg.dataset == 'sailvos3d':
            self.test_file_names = sorted(glob.glob(os.path.join(prefix, video_name, '*' + 'npz')))

    def __getitem__(self, idx):
        res = np.load(self.test_file_names[idx])
        
        semantic_res = res['semantic_res']
        instance_conf = res['instance_conf']
        instance_label = res['instance_label']
        instance_mask = res['instance_mask']
        weights = res['weights']
        kernel_merge = res['kernel_merge']
        # print(semantic_res.shape, instance_conf.shape, instance_label.shape, instance_mask.shape, weights.shape, kernel_merge.shape)
        query_feats = []
        for i in range(instance_label.shape[0]):
            query_feats.append(dict(query_feat_weights = weights[i],
                                    query_feat_kernel_merge = kernel_merge[i],
                                    cls_id=instance_label[i])) 
        
        
        return instance_mask, query_feats
