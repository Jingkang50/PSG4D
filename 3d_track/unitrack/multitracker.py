...
from .data.query_feat_tracklet import QueryFeatTube
...
class AssociationTracker3D(object):
    def __init__(self, tracker_cfg):
        self.tracker_cfg = tracker_cfg
        self.tracked_stracks = []  # type: list[STrack]
        self.lost_stracks = []     # type: list[STrack]
    ...
    def update(self, obs, query_feats, total_num_tubes_previous, yembs=None):
        torch.cuda.empty_cache()
        self.frame_id += 1
        activated_stracks = []
        refind_stracks = []
        ...
        t1 = time.time()
        embs = [dict_tmp['query_feat_weights'][np.newaxis, np.newaxis, :, np.newaxis] for dict_tmp in query_feats]
        # embs = [dict_tmp['query_feat_kernel_merge'][np.newaxis, np.newaxis, :, np.newaxis] for dict_tmp in query_feats]
        embs = np.concatenate(embs, axis=0)
        embs = torch.tensor(embs).cuda()
        # obs = torch.tensor(obs).cuda()
        detections = self.prepare_obs(obs, embs)
        ...
        for itracked, idet in matches:
            ...
            # update qf_tube ------------------------------------------------------------------------
            query_feat = query_feats[idet]
            # print(len(self.query_feat_tubes), track.track_id - 1 - total_num_tubes_previous)
            self.query_feat_tubes[track.track_id - 1 - total_num_tubes_previous].update(query_feat, self.frame_id)
            # ----------------------------------------------------------------------------------------
            if track.state == TrackState.Tracked:
                track.update(detections[idet], self.frame_id)
