...
from .data.single_video import LoadOutputsFromMask2Former, LoadOutputsDKNet

from .eval import trackeval
from .eval.mots.MOTSVisualization import MOTSVisualizer
from .utils import visualize as vis
from .utils import io as io
from .mask import MaskAssociationTracker, MaskAssociationTracker3D
...
def eval_seq_3d(tracker_cfg, video_name, total_num_tubes_previous): #TODO - remeber to add frame_rate to tracker_cfg

    tracker = MaskAssociationTracker3D(tracker_cfg) 
    timer = Timer()
    dataloader = LoadOutputsDKNet(video_name, tracker_cfg)
    results = []
    num_tubes_this_video = 0
    for frame_id, (obs, query_feats) in enumerate(dataloader):
        if frame_id % 20 == 0:
            logger.info('Processing frame {} ({:.2f} fps)'.format(
                frame_id, 1./max(1e-5, timer.average_time)))
        online_tlwhs = []
        ...
        if len(obs) == 0: # nothing in this frame!
            ...
            # run tracking
            timer.tic()
            online_targets, num_tubes_this_video = tracker.update(obs, query_feats, total_num_tubes_previous)
            for t in online_targets: # iterate obs in a frame
                tlwh = t.tlwh * tracker_cfg.common.down_factor
                tid = t.track_id
                mask = t.mask.astype(np.uint8)
    ...
    prefix = tracker_cfg.save_prefix
    save_root = os.path.join(prefix, video_name)
    print('Writing to: ', save_root)
    if not os.path.exists(save_root):
        os.makedirs(save_root, exist_ok=True)
    result_filename = osp.join(save_root, "mask.npy")
