...
from models.unitrack.model import resnet
from models.unitrack.model import hrnet
from models.unitrack.model import random_feat_generator
...
class AppearanceModel(nn.Module):
    def __init__(self, tracker_cfg):
        ...
        self.model = make_encoder(tracker_cfg).to(self.tracker_cfg.common.device)
    def forward(self, x):
        z = self.model(x)
        return z
...
def make_encoder(tracker_cfg):
    ...
    model_type = tracker_cfg.common.model_type
    if model_type == 'crw':
        net = resnet.resnet18()
        if osp.isfile(tracker_cfg.common.resume):
            ckpt = torch.load(tracker_cfg.common.resume)
            state = {}
        ...
        net = resnet.resnet50(pretrained=False)
        net_ckpt = torch.load(tracker_cfg.common.resume)
        ...
        net = load_uvc_model(tracker_cfg.common.resume)
    elif model_type == 'timecycle':
        net = load_tc_model(tracker_cfg.common.resume)
    elif model_type in SSL_MODELS:
        net = resnet.resnet50(pretrained=False)
        net_ckpt = torch.load(tracker_cfg.common.resume)
        partial_load(net_ckpt, net)
    elif 'hrnet' in model_type:
        net = hrnet.get_cls_net(model_type, return_stage=tracker_cfg.common.return_stage, pretrained=tracker_cfg.common.resume)
    elif model_type == 'random':
        net = random_feat_generator.RandomFeatGenerator(tracker_cfg)
    else:
        raise ValueError('Invalid model_type.')
    if hasattr(net, 'modify'):
        net.modify(remove_layers=tracker_cfg.common.remove_layers)

    if 'Conv2d' in str(net) and not tracker_cfg.common.infer2D:
        net = From3D(net)
    return net
