...
class DefaultFormatBundleWithDepth:
    ...
    def __call__(self, results):
        ...
        if 'dep_img' in results:
            dep_img = results['dep_img']
            if self.img_to_float is True and dep_img.dtype == np.uint8:
                # Normally, image is of uint8 type without normalization.
                # At this time, it needs to be forced to be converted to
                # flot32, otherwise the model training and inference
                # will be wrong. Only used for YOLOX currently .
            ...
            # add default meta keys
            # results = self._add_default_meta_keys(results) TODO - check if commented
            if len(dep_img.shape) < 3:
                dep_img = np.expand_dims(dep_img, -1)
            dep_img = np.ascontiguousarray(dep_img.transpose(2, 0, 1)) # HWC to CHW
            results['dep_img'] = DC(
                to_tensor(dep_img), padding_value=self.pad_val['img'], stack=True)
