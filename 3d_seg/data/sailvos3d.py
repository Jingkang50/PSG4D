...
class Dataset:
    ...
    def trainLoader(self):
        prefixs = []
        for line in open("/mnt/lustre/jkyang/PSG4D/dknet/dataset/sailvos3d/train.txt"):
            prefixs.append("/mnt/lustre/jkyang/PSG4D/sailvos3d/downloads/sailvos3d/" + line[:-1])
        self.images_train_path = []
        self.depth_train_path = []
        self.rage_matrices_train_path = []
        self.visible_train_path = []
        for prefix in prefixs:
            datapath=os.path.join(prefix, "rage_matrices")
            ids = sorted(os.listdir(datapath))
            ids = random.sample(ids, int(len(ids) / 1))
            for id in ids:
                self.images_train_path.append(prefix + "/images/" + id[:-3] + "bmp")
                self.depth_train_path.append(prefix + "/depth/" + id[:-3] + "npy")
                self.rage_matrices_train_path.append(prefix + "/rage_matrices/" + id[:-3] + "npz")
                self.visible_train_path.append(prefix + "/visible/" + id[:-3] + "npy")
    ...
    def valLoader(self):
        prefixs = []
        for line in open("/mnt/lustre/jkyang/PSG4D/dknet/dataset/sailvos3d/val.txt"):
            prefixs.append("/mnt/lustre/jkyang/PSG4D/sailvos3d/downloads/sailvos3d/" + line[:-1])
        ...
        for prefix in prefixs:
            datapath=os.path.join(prefix, "rage_matrices")
            ids = sorted(os.listdir(datapath))
            ids = random.sample(ids, int(len(ids) / 10))
            for id in ids:
                self.images_val_path.append(prefix + "/images/" + id[:-3] + "bmp")
                self.depth_val_path.append(prefix + "/depth/" + id[:-3] + "npy")
                self.rage_matrices_val_path.append(prefix + "/rage_matrices/" + id[:-3] + "npz")
                self.visible_val_path.append(prefix + "/visible/" + id[:-3] + "npy")
    ...
    def testLoader(self):
        ...
        for prefix in prefixs:
            ...
            ids = random.sample(ids, int(len(ids) / 10))
            for id in ids:
                self.images_path.append(prefix + "/images/" + id[:-3] + "bmp")
                self.depth_path.append(prefix + "/depth/" + id[:-3] + "npy")
                self.rage_matrices_path.append(prefix + "/rage_matrices/" + id[:-3] + "npz")
                self.visible_path.append(prefix + "/visible/" + id[:-3] + "npy")
        self.test_files = self.rage_matrices_path
        ...
        logger.info('Testing samples ({}): {}'.format(self.test_split, len(self.images_path)))
        self.superpoints = {}
        ...
        test_set = list(np.arange(len(self.rage_matrices_path)))
