...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        scores, scores_nn = attention(q, k, v, self.d_k, mask, self.dropout)
        ...
        return output, scores_nn.mean(1).squeeze()
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        x2 = self.norm_1(x)
        x2, scores = self.attn_1(x2,x2,x2,mask) 
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, xyz, features, batch_ids):
        ...
        pt_num = xyz.shape[0]
        scores = torch.zeros((pt_num, pt_num)).cuda().float()
        for i in range(batch_size):
            ...
            for i in range(self.N):
                x, batch_scores = self.layers[i](x, mask=None)
            
            batch_mask = batch_id.unsqueeze(-1).repeat(1, batch_id.size(0))
            scores[batch_mask, batch_mask.permute(1,0)] = batch_scores
