...
from .multitracker import AssociationTracker, AssociationTracker3D
...
class MaskAssociationTracker3D(AssociationTracker3D):
    def __init__(self, tracker_cfg):
        super(MaskAssociationTracker3D, self).__init__(tracker_cfg)
    ...
    def prepare_obs(self, obs, embs):
        ''' Step 1: Network forward, get detections & embeddings'''
        if obs.shape[0] > 0:
            boxes = np.zeros((obs.shape[0], 4))
            detections = [STrack(tlbr_to_tlwh(tlbrs), 1, f, self.buffer_size, mask, ac=True, use_kalman=False) \
                    for (tlbrs,mask,f) in zip(boxes, obs, embs)]
        else:
            detections = []
        return detections
