...
class Dataset:
    ...
    def trainLoader(self):
        ...
        for line in open(train_list_file):
            self.train_file_names += sorted(glob.glob(os.path.join(self.data_root, line[:-1], '*' + 'pth')))
    ...
    def valLoader(self):
        val_list_file = self.test_list_file
        self.val_file_names = []
        for line in open(val_list_file):
            self.val_file_names += sorted(glob.glob(os.path.join(self.data_root, line[:-1], '*' + 'pth')))
    ...
    def testLoader(self):
        ...
        for line in open(test_list_file):
            self.test_file_names += sorted(glob.glob(os.path.join(self.data_root, line[:-1], '*' + 'pth')))
        self.test_files = self.test_file_names
        import random
        if self.sampler_test != -1 and len(self.test_files) > self.sampler_test:
