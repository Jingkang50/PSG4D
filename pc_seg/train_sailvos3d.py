...
def init():
    ...
    os.system('cp train_sailvos3d.py {}'.format(backup_dir))
    ...
    if cfg.distributed:
        dist.init_process_group(backend="nccl", init_method=f'tcp://127.0.0.1:12246', rank=0, world_size=1)
...
if __name__ == '__main__':
    ...
    from test_sailvos3d import test
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if (utils.is_multiple(epoch, cfg.save_freq) or utils.is_power2(epoch)) and epoch > (cfg.prepare_epochs + cfg.semantic_epochs):
            if (cfg.distributed == False) or (dist.get_rank() == 0):
                ap_list = test(model, model_test_fn, test_dataloader, dataset, epoch, logger, cfg)
