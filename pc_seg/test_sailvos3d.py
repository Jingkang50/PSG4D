...
import utils_sailvos3d.eval as eval
import torch.distributed as dist  

# from data.scannetv2_inst import Dataset
from dataset.sailvos3d import Dataset
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader):
            ...
            if (epoch > cfg.prepare_epochs):
                ...
                # object_idxs = torch.nonzero(semantic_preds <= (cfg.classes-2)).view(-1)
                object_idxs = torch.nonzero(semantic_preds > 9).view(-1)
                ...
                for idx in range(batch_idxs.max()+1):
                    ...
                    if cfg.save_res:
                        os.makedirs(os.path.join(result_dir, 'segmentation_results', test_scene_name.split('/')[-2]), exist_ok=True)
                        semantic_np = semantic_preds[pts_batch_mask].cpu().numpy()
                        print("Saving to ", os.path.join(result_dir, 'segmentation_results', test_scene_name.split('/')[-2]), test_scene_name)
                        np.savez(os.path.join(result_dir, 'segmentation_results', test_scene_name.split('/')[-2], test_scene_name.split('/')[-1][:-4]), semantic_res = semantic_np,
